...
chart.get_default_x_axis().set_title('Countries').set_interval(0, len(countries) * 2)
...
for i, country in enumerate(countries):
    x_position = 0.75 + i * 1.5  # Adjust starting and spacing positions
    ...
    balloon.set_scale(0.005)  # Smaller scale for balloons
...
# Adjust balloon color based on energy data
...
# Move balloons and update lines
async def move_balloons():
    for year_idx, year in enumerate(years):
        ...
        for i, country in enumerate(countries):
            ...
            balloons[country].set_model_location(x_position, height, year_idx)  # Adjust for Z-axis
            balloons[country].set_color(color)
            
            # Update line series
            line_series[country].add([x_position], [height], [year_idx])
